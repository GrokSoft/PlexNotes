...
(function () {
    ...
    var serverPort = 8080;
    var dbDialect = 'sqlite';                                   // One of mysql, postgres, sqlite, mariadb oe mssql
    var dbName = 'plex-notes.sqlite';                           // must make sense with dbDialect
    ...
    // Detect if the datastore file exists
    var isNewDatabase = (fs.existsSync(dStore.getDbPath(dbName)) == false);
    if (isNewDatabase == true) {
        logger.log('info', "Datastore not found, new database will be created");
    }

    // Initialize the datastore interface & create and prime if does not exist
    dStore.init(dbDialect, dbName, logger).then(function () {
        ...
        prom.then(function () {
            if (isNewDatabase == true) {
                logger.log('info', "Created and primed the database, please restart the program, exiting");
                process.exit(0);            // end the process
            } else {
                logger.log('info', "Initialized database connection");
            }
        });
    });
    if (isNewDatabase == true) {
        logger.log('info', "returning from mainline while database is being setup");
        return 0;                           // end this execution path, leave process running
    }

    // Generate faux notes for testing
    if (options.generate > 0) {
        logger.log('info', 'Generate ' + options.generate + ' faux notes');
        dStore.createRandomNotes(options.generate).then(function () {
            var prom = wait.until(function () {
                var b = dStore.getGeneratedFlag();
                return b;
            });
            prom.then(function () {
                logger.log('info', "Created " + options.generate + " random notes, exiting");
                process.exit(0);            // end the process
            });
        });
    }
    if (options.generate > 0) {
        logger.log('info', "returning from mainline while faux notes are being generated");
        return 0;                           // end this execution path, leave process running
    }
